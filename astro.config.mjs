		...
		starlight({
			...
			sidebar: [
				...
				},
				{
					label: 'Scripting',
					collapsed: true,
					// Flat pages are listed explicitly so the nested "Engine subsystems"
					// group can carry a clean label. Starlight derives an autogenerated
					// subdirectory's label from the raw folder name ("engine-subsystems"),
					// which isn't presentable — so we name the group ourselves and only
					// autogenerate its contents. The bare slug strings pull their label
					// from each page's frontmatter, so the nav stays in sync with the pages.
					items: [
						'scripting/overview',
						'scripting/enscript-basics',
						'scripting/common-gotchas',
						'scripting/game-structure',
						{
							label: 'Engine subsystems',
							collapsed: true,
							items: [{ autogenerate: { directory: 'scripting/engine-subsystems' } }],
						},
					],
				},
				{
